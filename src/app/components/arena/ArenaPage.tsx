...
export function ArenaPage() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: T.bg, fontFamily: "'Inter', system-ui, sans-serif", overflow: "hidden" }}>
      ...
      {/* ══ SIDEBAR ══ */}
      <aside style={{ width: "66px", background: T.bg, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", alignItems: "center", padding: "0.875rem 0", flexShrink: 0, zIndex: 30 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.25rem", width: "100%" }}>
          <SideNavItem icon={<SkipForward size={16} />} label="Roll" active />
          <SideNavItem icon={<Gamepad2 size={16} />} label="Play" onClick={() => navigate("/games")} />
          <div style={{ position: "relative", width: "100%", display: "flex", justifyContent: "center" }}>
            <SideNavItem icon={<Bell size={16} />} label="Alerts" onClick={() => { setNotifOpen(!notifOpen); setNotifSeen(true); }} active={notifOpen} />
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem" }}>
          <button onClick={() => navigate("/profile")} style={{ width: "40px", height: "40px", borderRadius: "50%", background: `linear-gradient(135deg, ${T.accent}, #2ABDA6)`, border: `2px solid rgba(77,217,192,0.2)`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.72rem", fontWeight: 800, color: T.bg }}>
            {ME.initials}
          </button>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
        ...
        {/* HEADER */}
        <header style={{ height: "48px", background: T.bg, borderBottom: `1px solid ${T.borderUp}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 1.25rem", flexShrink: 0, zIndex: 20 }}>
          <div>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
          ...
          {callState === "calling" && (
            <div style={{ flex: 1, position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: "1rem", right: "1rem", width: "148px", height: "110px", borderRadius: "14px", border: `2px solid rgba(77,217,192,0.2)`, overflow: "hidden", zIndex: 20 }}>
                <CameraView videoRef={ownVideoRef} isMuted videoOn={videoOn} camError={camError} label="You" style={{ position: "absolute", inset: 0 }} />
          ...
          {chatOpen && isInCall && (
            <>
              ...
              <div
                ...
                style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "300px", background: "#FFFFFF", borderLeft: "1px solid #E5E5E5", zIndex: 35, display: "flex", flexDirection: "column", animation: "chat-panel-in 0.2s cubic-bezier(0.22,1,0.36,1)" }}
              >
                {/* Chat header */}
                ...
                <div style={{ padding: "0.75rem", borderTop: "1px solid #F0F0F0", flexShrink: 0, background: "#FFFFFF", display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    style={{ width: "34px", height: "34px", borderRadius: "50%", background: chatInput.trim() ? T.accent : "#EBEBEB", border: "none", cursor: chatInput.trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
                  >
                    <Send size={14} color={chatInput.trim() ? "#0F1923" : "#AAAAAA"} />
                  </button>
                </div>
              </div>
            </>
        ...
        {/* ── CONTROL BAR ── */}
        <div style={{ height: "78px", background: T.bg, borderTop: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 1.5rem", flexShrink: 0, position: "relative", zIndex: 10 }}>
      ...
      {pendingChallenges.map((c) => (
        <div key={c.id} style={{ position: "fixed", top: "16px", right: "16px", zIndex: 9999, display: "flex", alignItems: "center", gap: "10px", background: T.surface, border: `1px solid ${T.border}`, borderRadius: "10px", padding: "12px 14px", width: "256px", animation: "challenge-toast-in 0.22s cubic-bezier(0.16,1,0.3,1)" }}>
          ...
          <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
            <button onClick={() => setChallenges((p) => p.map((x) => x.id === c.id ? { ...x, dismissed: true } : x))} style={{ width: "28px", height: "28px", background: T.greenDark, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "6px" }} title="Accept">
              <svg width="13" height="13" viewBox="0 0 14 14" fill="none"><polyline points="2,7 5.5,10.5 12,3.5" stroke={T.greenBright} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
            <button onClick={() => setChallenges((p) => p.map((x) => x.id === c.id ? { ...x, dismissed: true } : x))} style={{ width: "28px", height: "28px", background: T.border, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "6px" }} title="Decline">
              <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><line x1="1.5" y1="1.5" x2="10.5" y2="10.5" stroke="#4A6070" strokeWidth="1.7" strokeLinecap="round"/><line x1="10.5" y1="1.5" x2="1.5" y2="10.5" stroke="#4A6070" strokeWidth="1.7" strokeLinecap="round"/></svg>
            </button>
          </div>
        </div>
      ))}
...
/* ══ Sub-components ══ */
function SideNavItem({ icon, label, active = false, onClick }: { icon: React.ReactNode; label: string; active?: boolean; onClick?: () => void; }) {
  return (
    <button onClick={onClick} style={{ width: "46px", padding: "0.55rem 0", borderRadius: "12px", background: active ? "#1A3A4A" : T.surface, border: active ? `1px solid #2A4A5A` : `1px solid ${T.border}`, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "0.2rem", color: active ? T.accent : T.iconInact, transition: "color 0.15s" }}>
      {icon}
